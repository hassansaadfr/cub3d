...
t_coord		*ft_calc_pos_in_map(t_vars *vars)
{
	...
	coord->x = vars->player.player_pos.x / MAP_CUBE_SIZE;
	coord->y = vars->player.player_pos.y / MAP_CUBE_SIZE;
	return (coord);
}
...
void		ft_init_minimap(t_vars *vars)
{
	...
	vars->player.player_pos.x = x_px * MAP_CUBE_SIZE + (MAP_CUBE_SIZE / 2);
	vars->player.player_pos.y = y_px * MAP_CUBE_SIZE + (MAP_CUBE_SIZE / 2);
