...
/*   Updated: 2021/02/10 19:15:51 by hsaadaou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_south(t_vars *vars)
{
	...
	vars->player_pos.y += MAP_PLAYER_SIZE + 1;
	pos = ft_calc_pos_in_map(vars);
	c = vars->c->map[pos->y][pos->x];
	if (c == '0')
	{
		vars->player_pos.y -= MAP_PLAYER_SIZE;
		vars->c->player_pos = pos;
		draw_minimap(vars);
		draw_player(vars, &vars->player_pos);
		ft_draw_img(vars, 0, 0);
...
void	move_west(t_vars *vars)
{
	...
	vars->player_pos.x += MAP_PLAYER_SIZE + 1;
	pos = ft_calc_pos_in_map(vars);
	c = vars->c->map[pos->y][pos->x];
	if (c == '0')
	{
		vars->player_pos.x -= MAP_PLAYER_SIZE;
		vars->c->player_pos = pos;
		draw_minimap(vars);
		draw_player(vars, &vars->player_pos);
		ft_draw_img(vars, 0, 0);
